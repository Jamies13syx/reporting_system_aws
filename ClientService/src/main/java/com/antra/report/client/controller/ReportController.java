...
public class ReportController {
    ...
    public ResponseEntity<GeneralResponse> createReportDirectly(@RequestBody @Validated ReportRequest request) throws ExecutionException, InterruptedException {
        log.info("Got Request to generate report - sync: {}", request);
        request.setDescription(String.join(" - ", "Sync", request.getDescription()));
        return ResponseEntity.ok(new GeneralResponse(reportService.generateReportsSync(request).get()));
    }
   ...
   @DeleteMapping("/report/{reqId}")
    public ResponseEntity<GeneralResponse> deleteFile(@PathVariable String reqId) {
        log.debug("Got Request to delete report - reqid: {}", reqId);
        boolean deleted = reportService.deleteReportById(reqId);
        if (deleted) {
            return ResponseEntity.ok(new GeneralResponse()); // if delete success, return 200
        }
        return new ResponseEntity<>(new GeneralResponse(), HttpStatus.BAD_REQUEST); // if delete fail, return 400
