...
//        request.setReqId("Req-"+ UUID.randomUUID().toString());
    ...
    public CompletableFuture<ReportVO> generateReportsSync(ReportRequest request) {
        ...
        new CompletableFuture<ReportVO>();
...
//            InputStream is = restTemplate.execute(, HttpMethod.GET, null, ClientHttpResponse::getBody, fileId);
    ...
    public boolean deleteReportById(String Id) {
        try {
            reportRequestRepo.deleteById(Id);
            log.info("local repo delete success");
        }
        catch (EmptyResultDataAccessException exception) {
            log.error("No such entity");
            return false;
        }
        RestTemplate restTemplate = new RestTemplate();
        try {
            ResponseEntity<ExcelResponse> deletedExcel = restTemplate.exchange("http://localhost:8888/excel/" + Id,
                    HttpMethod.DELETE, null, ExcelResponse.class);
            ResponseEntity<ExcelResponse> deletedPDF = restTemplate.exchange("http://localhost:9999/pdf/" + Id,
                    HttpMethod.DELETE, null, ExcelResponse.class);
            log.debug("Excel delete -> " + deletedExcel.getBody().isFailed());
            log.debug("PDF delete -> " + deletedPDF.getBody().isFailed());
            return !deletedExcel.getBody().isFailed() && !deletedPDF.getBody().isFailed();
        }
        catch (Exception e) {
            log.error("delete process error -> " + e);
        }
        return false;
