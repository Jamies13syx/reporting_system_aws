...
class ReportingSystemApplicationTests {
    ...
    public void testSaveToRedis() {
        request.setReqId("testonly");
        request.setSubmitter("tester");
        request.setDescription("test");
        request.setHeaders(new ArrayList<>());
        request.setData(new ArrayList<>(new ArrayList<>()));
        ExcelFile testFile = excelService.generateFile(request, false);
        assertTrue(testFile != null);
    }

    @Test
    public void testDeleteFromRedis() throws FileNotFoundException {
        ExcelFile deleted = excelService.deleteFile("testonly");
